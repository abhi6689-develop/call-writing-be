...
class OptionDataApp(EWrapper, EClient):
    def __init__(self):
        ...
        self.data_dict = {"bid_price": [], "bid_size": []}

    def error(
        self, reqId, errorCode, errorString, advanced_order_reject_json: str = ""
    ):
    ...
    def create_option_df(self):
        option_chains_df = pd.DataFrame(df_dict).drop_duplicates(
            subset=["exchange"], keep="last"
        )
...
def get_in_the_money_calls(portfolio_df, option_df, expiry):
    ...
    merged_df = portfolio_df.merge(
        option_df, left_on="ticker", right_on="tradingClass", how="inner"
    )
    ...
    weekly_expirations, monthly_expirations = zip(
        *merged_df["expirations"].apply(lambda x: classify_expiration(x, expiry))
    )
            ...
            in_the_money_calls.append(
                {
                    ...
                    "weekly_bids": [
                        uniform(0.9, 1.1) for _ in row["weekly_expirations"]
                    ],
                    "monthly_bids": [
                        uniform(0.9, 1.1) for _ in row["monthly_expirations"]
                    ],
...
def get_option_chain():
    ...
    portfolio_data = pd.DataFrame(
        {
            ...
            "quantity": [150, 50, 120, 3],
            "price": [500, 100, 200, 300],
        }
    )

    # Filter out stocks with quantities below 100
    eligible_stocks = portfolio_data[portfolio_data["quantity"] >= 100].to_dict(
        orient="records"
    )
    ...
    for stock in eligible_stocks:
        ...
        calls_df = get_in_the_money_calls(
            portfolio_df, option_df, expiry=int(today_date)
        )
        print(calls_df)
